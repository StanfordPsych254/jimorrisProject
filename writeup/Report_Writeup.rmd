...
We planned a sample of 400 participants from mTurk. We excluded the sample to only U.S. participants.
...
In the original study, the authors used 1b) mTurk, N = 151; 1c) mTurk, N = 604. This study had a sample size of 400 and also used mTurk.
...
qplot(wtp,data=d.final) +
  facet_grid(cond~SES_cat_3)

#Author Plot
Sum_d = d.final %>%
  group_by(cond,SES_cat_3) %>%
  summarize(N = length(wtp),
            WTP = mean(wtp, na.rm=TRUE),
            sd   = sd(wtp, na.rm=TRUE),
            se   = sd / sqrt(N))

ggplot(Sum_d,aes(x=SES_cat_3, y=WTP, fill=cond)) + 
