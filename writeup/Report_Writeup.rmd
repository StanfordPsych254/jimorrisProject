...
contrasts(d.final$SES_cat_1)=cbind(High=c(1,0))
contrasts(d.final$SES_cat_2)=cbind(High=c(1,0))
contrasts(d.final$SES_cat_3)=cbind(High=c(1,0))
...
reg_c = lm(wtp ~ Frame * SES, data = d.final); kable(summary(reg_c)$coef)
```
...
reg_c_b.1 = lm(wtp ~ Frame * SES * beer_like, data = d.final); kable(summary(reg_c_b.1)$coef)
...
reg_c_b.3 = lm(wtp ~ Frame * SES, data = d.beer); kable(summary(reg_c_b.3)$coef)
```
